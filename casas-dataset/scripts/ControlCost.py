...
class ControlCost:
    ...
    def possible_control_costs(self):
        num_activities = len(self.activities)  # Does NOT include index 0

        # Map activity ID to its zone
        activity_zone_map = {
            int(self.activities["Activity ID"][i]): int(self.activities["Zone ID"][i])
            for i in range(num_activities)
        }

        # Initialize parameter arrays with dummy index 0
        activity_zone_temp_setpoint = [0]
        activity_zone_co2_setpoint = [0]
        activity_pp_co2 = [0] + self.activities['CO2 Emission by Occupant (CFM)'].tolist()
        activity_pp_heat = [0] + self.activities["Heat Radiation by Occupant (W)"].tolist()
        activity_load = [0]
        activity_zone_volume = [0]

        for i in range(1, num_activities + 1):
            zone_id = activity_zone_map[i]
            activity_zone_temp_setpoint.append(zone_temp_setpoint[zone_id])
            activity_zone_co2_setpoint.append(zone_co2_setpoint[zone_id])
            activity_zone_volume.append(self.zone_volume[zone_id])
            activity_load.append(
                sum(self.benign_activity_appliances_map[i] * self.energy_appliances)
            )

        # Include index 0 in combinations
        all_indexes = list(range(0, num_activities + 1))

        # Generate all 2-activity combinations (including index 0)
        pair_indexes = list(combinations(all_indexes, 2))

        # Total samples = all pairs + all singles (including index 0)
        total_samples = len(pair_indexes) + len(all_indexes)
        unique_samples = np.zeros((total_samples, num_activities + 1), dtype=int)

        # Fill pairs (set 1s)
        for i, (a1, a2) in enumerate(pair_indexes):
            unique_samples[i][a1] = 1
            unique_samples[i][a2] = 1

        # Fill singles (set 2s)
        for i, idx in enumerate(all_indexes):
            unique_samples[len(pair_indexes) + i][idx] = 2

        # Compute and store control costs
        self.dict_control_cost = dict()

        for sample in unique_samples:
            activity_zone_occupant = sample.tolist()
            key = self.to_tuple(sample)
            self.dict_control_cost[key] = self.control_cost(
                activity_zone_occupant,
                activity_zone_temp_setpoint,
                activity_zone_volume,
                activity_pp_co2,
                activity_pp_heat,
                activity_load,
                activity_zone_co2_setpoint,
                control_time
            )    
